...

{
  "intent_candidates": [
    {
      "intent": "short_uav_action_name",
      "target": "visible target or scene element",
      "applicability_score": 0.0,
      "feasibility_score": 0.0,
      "final_score": 0.0,
      "reason": "short reason"
...
def extract_json(text):
    ...
    if not match:
        ...
        raise ValueError(f"No JSON object found in VLM output. Raw output saved to: {debug_path}")
...
def normalize_scores(result):
    ...
    for item in result.get("intent_candidates", []):
        ...
        applicability = max(0.0, min(1.0, applicability))
        feasibility = max(0.0, min(1.0, feasibility))
        final_score = round((applicability + feasibility) / 2.0, 3)

        cleaned.append({
            "intent": str(item.get("intent", "unknown_intent")),
            "target": str(item.get("target", "visible_scene")),
            "applicability_score": round(applicability, 3),
            "feasibility_score": round(feasibility, 3),
            "final_score": final_score,
            "reason": str(item.get("reason", "Generated by VLM from image and belief state.")),
        })

    cleaned.sort(key=lambda x: x["final_score"], reverse=True)

    return {
        "intent_candidates": cleaned[:3]
    }
...
def generate_vlm_intents(image_path, belief_json_path, model, processor):
    ...
    prompt = f"""
You are a UAV intent-generation module.

Use the aerial image and the perception belief state to generate possible UAV intents.

Definitions:
- applicability_score: how relevant the intent is to the visible scene.
- feasibility_score: how practical and safe the intent appears.
- final_score: average of applicability_score and feasibility_score.
...
"""
    ...
    result["source_image"] = str(image_path)
    result["belief_state"] = str(belief_json_path)
    result["method"] = "vlm_qwen2_5_vl_3b_scene_level"
